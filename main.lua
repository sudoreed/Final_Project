...
-- function setupOrbCollider(wf)
--     Hud.collider = world:newCircleCollider(100, 400, 0)
--     Hud.collider:setCollisionClass('Orb')
--     Hud.collider:setFixedRotation(true)
...
function setupAnimations(anim8)
    ...
    Hud.orb_animations.orb = {
        animate = anim8.newAnimation(Hud.grid('1-4', 1), 0.2),
...
function setupMap()
    ...
    if gameMap.layers["Orb"] then
        for _, obj in pairs(gameMap.layers["Orb"].objects) do
            ...
            Orb:setType('static')
            Orb:setCollisionClass('Orb')
            table.insert(OrbColliders, Orb)
        end
    end
end
...
function love.draw()
    ...
    -- Draw the orb animation
    if Hud.anim and Hud.anim.orbsheet then
        Hud.anim.animate:draw(
            Hud.anim.orbsheet,
            500, 410, 0,
            0.37, 0.37, Hud.orb_frame_h / 2, Hud.orb_frame_h / 2
        )
    end
    ...
    -- Set color of print statements to black and change back to default after
    love.graphics.setColor(0,0,0)
    -- Draw the timer on the screen
    love.graphics.print("Time: " .. formattedTime, 530, 10, 0, 2, 2)
    -- Draw number orb counter
    love.graphics.print("Orbs: " .. Hud.orb_count, 1100, 15, 0, 2, 2)
